...
Data Analysis Python Pandas Data Manipulation Data Cleaning
...
select 
  substring(date_joined, 1, 7) as month_joined,
  count(month_joined)

from (
  select 
    case when date_joined is null then '2009-12-01'
    else date_joined end as date_joined,
    ROW_NUMBER() OVER (PARTITION BY employee_id) as num,
  from employees
)
